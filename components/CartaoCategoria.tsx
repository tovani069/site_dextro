    ...
    <article className="cartao group flex flex-col p-6 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_40px_-24px_rgba(15,23,42,0.35)]">
      <div className="mb-7 flex items-start justify-between">
        <span className="flex h-[52px] w-[52px] items-center justify-center rounded-2xl bg-tinta-100 dark:bg-white/5">
          <FolderOpen className="h-[22px] w-[22px] text-tinta-900 dark:text-white" strokeWidth={2} />
        </span>
        <span className="rounded-full bg-tinta-100 px-2.5 py-1 text-[10px] font-bold tracking-[0.08em] text-tinta-400 uppercase dark:bg-white/5 dark:text-tinta-400">
          {quantidade} {quantidade === 1 ? "item" : "items"}
        </span>
      </div>

      <h3 className="font-display text-[21px] leading-tight font-bold text-tinta-800 capitalize dark:text-white">
        {nome}
      </h3>
      <p className="mt-2 text-[14px] text-tinta-400 italic">{descricao}</p>
